...
const SkillsArray = (props) => {
    ...
    useEffect(() => {
        Axios.get('http://api.monsite80.fr:3003/skillsArray').then((response) => {
            response ? array(response.data.data) : console.log('error');
        })
    }, [])

    useEffect(() => {
        Axios.get('http://api.monsite80.fr:3003/projects').then((response) => {
            response ? setProjects(response.data.data) : console.log('error');
        })
    }, [])

    useEffect(() => {
        Axios.get('http://api.monsite80.fr:3003/estValide').then((response) => {
